...
| **Kiro CLI**             | Spec-driven development, autonomous coding | `curl -fsSL https://cli.kiro.dev/install \| bash`<br>See [Kiro CLI](https://kiro.dev/cli/)        | `kiro`           | **Amazon's spec-driven agentic assistant.** Successor to CodeWhisperer. Enforces specifications before coding, smart hooks for automated workflows, custom agents, MCP integration, and autonomous multi-day coding sessions. |
